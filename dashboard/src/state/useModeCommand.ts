...
import { setMode, estop, recover } from './roverCommands';

const TIMEOUT_MS = 1500;

export type PendingIntent = 'manual' | 'safe' | 'estop' | 'clearing' | null;
...
export function useModeCommand(roverId: string, currentMode: DisplayMode) {
  ...
  const requestMode = useCallback((t: 'manual' | 'safe') => { arm(t); setMode(roverId, t); }, [arm, roverId]);
