...
import (
	...
	"github.com/waypointos/waypoint/modules/survey/internal/uiserve"
...
func setup(m *wpmodule.M) error {
	...
	// Panel backend: wired before the platform mirrors so a persisted
	// waypoint override lands while the engine is still guaranteed IDLE.
	stateDir := filepath.Dir(cfg.LogDir)
	ui := uiserve.New(m, eng, uiserve.Options{
		LogDir:       cfg.LogDir,
		SnapDir:      filepath.Join(stateDir, "snaps"),
		OverridePath: filepath.Join(stateDir, "waypoints_override.json"),
		Baseline:     uiserve.Baseline{Waypoints: cfg.Waypoints, TagIDs: cfg.TagIDs, Start: cfg.Start},
	})
	if err := ui.LoadOverride(); err != nil {
		slog.Warn("survey: waypoint override not applied", "err", err)
	}
	eng.SetPoseListener(ui.OnPose)
	if proc != nil {
		eng.SetSenseListener(func(wp, id int) { go saveSnap(ui, proc, wp, id) })
	}
	if _, err := m.Subscribe(m.Subject("ui.req"), func(msg *natsgo.Msg) { ui.HandleReq(msg.Data) }); err != nil {
		return err
	}
	go ui.Run(ctx)
...
// saveSnap runs off the vision goroutine: path prep plus a child stdin write.
func saveSnap(ui *uiserve.Server, proc *vision.Proc, wp, id int) {
	path, err := ui.SnapPath(wp, id)
	if err == nil {
		err = proc.Snap(path)
	}
	if err != nil {
		slog.Warn("survey: snapshot", "err", err)
	}
}
