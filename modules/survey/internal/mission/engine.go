...
type Engine struct {
	...
	// UI-facing progress; the mission path never reads these.
	wpReached  []bool
	wpDetected []int // per waypoint, -1 = none
	lastDet    *DetectionMark
	startedAt  time.Time
	onPose     func(nav.Pose)
	onSense    func(wp, id int)
}
...
func (e *Engine) OnOdometry(vx, yawRate float64, now time.Time) {
	...
	e.measVX = vx
	e.lastOdoAt = now
	cb, pose := e.onPose, e.pose
	e.mu.Unlock()
	// Outside the lock so the listener may call back into the engine.
	if cb != nil {
		cb(pose)
	}
}
...
func (e *Engine) OnDetections(dets []Detection, now time.Time) {
	...
	tag := e.legs[e.li].tag
	dwellEmpty := len(e.senseCounts) == 0
	firstID := -1
	for _, d := range dets {
	...
	cb, wp := e.onSense, e.legs[e.li].wpIndex
	e.mu.Unlock()
	// First accepted sighting of the dwell: the marker just came into view,
	// the best snapshot timing.
	if cb != nil && dwellEmpty && firstID >= 0 {
		cb(wp, firstID)
	}
}
...
func (e *Engine) arm(now time.Time) {
	...
	e.li = 0
	e.wpReached = make([]bool, len(e.p.Waypoints))
	e.wpDetected = make([]int, len(e.p.Waypoints))
	for i := range e.wpDetected {
		e.wpDetected[i] = -1
	}
	e.lastDet = nil
	e.startedAt = now
...
func (e *Engine) arrive(now time.Time, lg leg) {
	...
	if lg.wpIndex >= 0 && lg.wpIndex < len(e.wpReached) {
		e.wpReached[lg.wpIndex] = true
	}
	if lg.sense {
...
func (e *Engine) tickSense(now time.Time) {
	...
	if ok {
		e.lastDetectedID = id
		if lg.wpIndex >= 0 && lg.wpIndex < len(e.wpDetected) {
			e.wpDetected[lg.wpIndex] = id
		}
		e.lastDet = &DetectionMark{WP: lg.wpIndex, ID: id, T: now}
